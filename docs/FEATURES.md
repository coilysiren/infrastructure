...
- **Host freshen** - `coily ansible-freshen` brings a host up to date across the coilysiren surface via `playbooks/freshen.yml` (Homebrew + agent-compose + repo-layout reconcile + git remote-sync sweep). Dry-runs by default (`--check --diff`); `action=apply` converges; `tags=<csv>` scopes to one role. The Ansible port of `agentic-os-kai/scripts/up-to-date.py`.
