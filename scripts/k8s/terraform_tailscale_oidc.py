...
provider auto-rotates short-lived tokens it mints) and a GitHub PAT
into the provider env from SSM. Plaintext values are passed via env,
never echoed or persisted. See docs/tailscale-oidc.md.
...
def main():
    ...
    gh_token = ssm_client.get_parameter(
        Name="/github/pat",
        WithDecryption=True,
    )["Parameter"]["Value"]
    env = os.environ.copy()
